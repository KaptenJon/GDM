    ...
    public class Sum : ITool
    {
        ...
        public void Apply(IModel model, PluginSettings pluginSettings, ILog log, IStatus status)
        {
            ...
            foreach (var machine in i)
            {
                ...
                var row = targettable.AsEnumerable().FirstOrDefault(t => Equals(t["Operation"].ToString(), machine.Key.ToString()));
                if (row == null)
                {
                ...
                row[settings.TargetColumn] = observations.Sum();
