...
{
    internal class EvaluateFormula
    {
        #region Fields
        ...
        public EvaluateFormula()
        {
            ...
            {
                _decimalSeparator =
                    Char.Parse(CultureInfo.CurrentCulture.NumberFormat.NumberDecimalSeparator);
            }
        ...
        public EvaluateFormula(char decimalSeparator)
        {
        ...
        private string LexicalAnalysisInfixNotation(string expression, ref int pos)
        {
            ...
            if (_supportedOperators.ContainsKey(token.ToString()))
            {
                ...
                switch (token.ToString())
                {
                    ...
                    default:
                        return _supportedOperators[token.ToString()];
                }
            }
            else if (Char.IsLetter(token[0])
                     || _supportedFunctions.ContainsKey(token.ToString())
                     || _supportedConstants.ContainsKey(token.ToString()))
            {
                ...
                if (_supportedFunctions.ContainsKey(token.ToString()))
                {
                    return _supportedFunctions[token.ToString()];
                }
                else if (_supportedConstants.ContainsKey(token.ToString()))
                {
                    return _supportedConstants[token.ToString()];
                }
                else
                {
            ...
            else if (Char.IsDigit(token[0]) || token[0] == _decimalSeparator)
            {
                ...
                if (pos < expression.Length
                    && expression[pos] == _decimalSeparator)
                {
        ...
        #region CalculateRows expression in RPN

        /// <summary>
        /// CalculateRows expression in reverse-polish notation
        /// </summary>
